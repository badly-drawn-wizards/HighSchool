...
void main () {
	gl_FragColor = cogs(rotate(get_pos()+vec2(.4,0),-TIME/5.), TIME/3.) ? vec4(.5*(1.+sin(time/500.)), 0 , 0, 0) :
						cogs(rotate(get_pos()+vec2(-.5,+.4)-OFFSET/3., -TIME)*3., TIME) ? vec4(0, .25*(1.+sin(time/500.+PI/3.)), 0, 0) : 
						cog_array(get_pos()-OFFSET, TIME/3., .1, .01, 9) ? vec4(0, 0, .1*(1.+sin(time/500.+2.*PI/3.)), 0) : vec4(0);
}